...
float4 Fog(float eyeDepth, float2 screenuv)
{
	...
	uvw.xy += (MNoise(screenuv, _RandomSeed) * 2 - 1) / (_FroxelSize.xy) * 0.7;
	float4 vol = _VolumeTex.SampleLevel(bilinearClampSampler, uvw, 0);
...
float4 frag_volume(v2f i) : SV_TARGET
{
    ...
    float depth = _DepthTex.SampleLevel(bilinearClampSampler, uv, 0);
