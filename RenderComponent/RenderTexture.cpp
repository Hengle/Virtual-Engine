			...
			mScissorRect({ 0, 0, (int)width, (int)height })
		{
			...
			if (rtFormat.usage == RenderTextureUsage::ColorBuffer)
			{
				...
				case TextureDimension::Tex2D:
					...
					device->CreateRenderTargetView(Resource.Get(), &rtvDesc, rtvHeap.hCPU(0));
					break;
			...
			else
			{
				...
				if (mFormat != DXGI_FORMAT_UNKNOWN)
				{
					...
					case TextureDimension::Tex2D:
						...
						device->CreateDepthStencilView(Resource.Get(), &dsvDesc, rtvHeap.hCPU(0));
						break;
					default:
						dsvDesc.Flags = D3D12_DSV_FLAG_NONE;
		...
		void RenderTexture::BindRTVToHeap(DescriptorHeap * targetHeap, UINT index, ID3D12Device * device, UINT slice)
		{
			if (usage == RenderTextureUsage::ColorBuffer)
			{
				...
				case TextureDimension::Tex2D:
					...
					rtvDesc.Texture2D.MipSlice = 0;
					rtvDesc.Texture2D.PlaneSlice = 0;
					device->CreateRenderTargetView(Resource.Get(), &rtvDesc, targetHeap->hCPU(index));
					break;
